...
import { Card, Button, Tag, Space, Spin, Empty, Badge, Typography, Table, Drawer, Checkbox, Dropdown, Popover, App } from 'antd';
...
export default function LabelPage() {
  ...
  const fetchTask = async () => {
    try {
      ...
      if (data.success) {
        ...
        await fetchDataEntries(data.data.data_object_id);
  ...
  const fetchDataEntries = async (dataObjectId: number) => {
    setDataLoading(true);
    try {
      ...
      if (data.success) {
        ...
        const entries = (data.data?.list || []).map((entry: any, index: number) => ({
          ...entry,
          _key: entry.id ? String(entry.id) : `entry-${index}`,
        }));
  ...
  const getEntryResults = (entry: DataEntry) => {
    const entryId = entry.id ? String(entry.id) : entry._key;
    ...
    try {
      const response = await fetch(`/api/labeling-tasks/${taskId}/results`, {
        ...
        body: JSON.stringify({
          ...
          data_entry_id: entry.id ? String(entry.id) : entry._key,
          tag_id: tagId,
        }),
      });
      const data = await response.json();
  ...
  const handleBatchTag = async (tagId: number) => {
    ...
    for (const entryId of selectedRowKeys) {
      try {
        const response = await fetch(`/api/labeling-tasks/${taskId}/results`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const labeledCount = dataEntries.filter(entry => getEntryResults(entry).length > 0).length;
  const progress = dataEntries.length > 0 ? (labeledCount / dataEntries.length) * 100 : 0;
  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      >
        ...
        <div style={{ marginBottom: '16px' }}>
          <Space>
            <Text>进度：{labeledCount} / {dataEntries.length}</Text>
