...
| `versionNumber` | string | 否 | 自定义版本号（如 `v1.0`、`v2.5`、`release-2024` 等）。如果不提供，系统自动生成递增版本号（格式：`v1`, `v2`, ...） |
...
| HTTP 状态码 | 错误说明 |
|------------|---------|
| 400 | 请求参数错误（如 `versionNumber` 为空字符串） |
| 401 | 认证失败 |
| 403 | 权限不足 |
| 409 | 版本号冲突（指定的 `versionNumber` 已存在） |
...
1. **自动生成**（默认）：如果不提供 `versionNumber` 参数，系统会自动生成递增的版本号，格式为 `v1`, `v2`, `v3` ...
2. **自定义版本号**：在请求体中提供 `versionNumber` 参数（字符串），系统会使用指定的版本号

**注意事项**：
- 自定义版本号可以是任意非空字符串，如 `v1.0`、`v2.5.1`、`release-2024` 等
- 同一配置下版本号不能重复，如果指定的版本号已存在，会返回 409 错误
